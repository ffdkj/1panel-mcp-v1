...
> 注意这是**面板密钥**，和客户端连接 MCP 用的**访问令牌**是两回事 ——
> 见 [两个密钥的分工](#两个密钥的分工)。客户端设备只需要后者。
...
本服务涉及**两个不同的密钥**，职责完全分离 —— **客户端设备只需要其中一个**：

|  | 面板 API 密钥 | MCP 访问令牌 |
|---|---|---|
| 变量 | `ONEPANEL_API_KEY` | `MCP_TOKEN` |
| 作用 | 服务端调用 1Panel `/api/v1/*` 时的身份 | 客户端连接 MCP 端点时的身份 |
| 配在哪 | **只在本机** `.env.local` | 服务器 + **每台客户端** |
| 怎么来 | 1Panel 面板 → 面板设置 → API 接口 → 生成 | 自己生成一串随机字符 |
| 客户端要吗 | **不需要** | 需要 |
| 泄漏后果 | 等于交出面板管理员权限 | 只能通过 MCP 调用工具 |

```
                ONEPANEL_API_KEY                MCP_TOKEN
              （只在本机，不出门）            （发给各客户端设备）
  ┌────────┐  ◄───────────────────  ┌───────────────┐  ◄──────────────  ┌──────────┐
  │ 1Panel │    127.0.0.1:36437     │ 1panel-mcp-v1 │  100.x.y.z:8790   │ opencode │
  │  面板  │                        │   HTTP 服务   │                   │ 平板/手机 │
  └────────┘                        └───────────────┘                   └──────────┘
```

`.env.local` 里两个都写，各司其职：

```bash
ONEPANEL_API_KEY=<面板生成的密钥>      # 面板设置 → API 接口
MCP_TOKEN=<你自己生成的随机串>          # 客户端拿这个连
```
...
**只设 `MCP_TOKEN` 而不设 `ONEPANEL_API_KEY` 时，服务会把同一个值同时当作面板密钥使用**
（单机快速起步时方便）。代价是每台客户端都持有了面板管理员密钥 —— 一台设备被攻破就等于
面板被接管。要分开就两个都设。

启动横幅会告诉你当前是哪种模式：

```
面板密钥来源 ONEPANEL_API_KEY      ← 已分开，推荐
面板密钥来源 MCP_TOKEN（回退）      ← 未设 ONEPANEL_API_KEY，两者共用同一个值
```
...
| 现象 | 原因 | 处理 |
|---|---|---|
| 客户端连接返回 `401`，日志写「令牌不匹配」 | 客户端配置里的令牌不对（常见：把文档占位符原样抄了，或 `{env:VAR}` 的变量没设） | 日志会区分「未携带令牌」和「令牌不匹配」；按 [两个密钥的分工](#两个密钥的分工) 确认客户端用的是 `MCP_TOKEN` |
| 日志里出现 `/.well-known/oauth-*`、`/register` 请求 | 客户端在走 OAuth 发现流程，说明它收到的令牌没通过校验 | 本服务不支持 OAuth，这些路径返回 404；按上一条检查令牌 |
| **重启服务后所有工具都报 `401 API 接口密钥错误`** | 改了 `MCP_TOKEN` 却**没设 `ONEPANEL_API_KEY`**，回退逻辑把这个值当成了面板密钥 | 两个都要设，见 [两个密钥的分工](#两个密钥的分工)；启动横幅的「面板密钥来源」会显示是 `ONEPANEL_API_KEY` 还是 `MCP_TOKEN（回退）` |
| `401 API 接口密钥错误`（其它情况） | 把 API Key 当成了 Token | 确认用的是 md5 签名（本 server 已自动处理） |
